resource "aws_lambda_function" "lambda_pro_interase_rate" {
...
resource "aws_lambda_function" "lambda_pro_insurance" {
  count         = var.environment == "pro" ? 1 : 0
...
resource "aws_lambda_function" "lambda_pro_customer" {
  count         = var.environment == "pro" ? 1 : 0
...
resource "aws_lambda_function" "lambda_pro_loans" {
  count         = var.environment == "pro" ? 1 : 0
  ...
  environment {
    variables = {
      ...
      "TABLE_DYNAMO" = "/pica/${var.environment}/loan/table-dynamo"
    }
  }
...
resource "aws_lambda_function" "lambda_pro_calculate" {
  count         = var.environment == "pro" ? 1 : 0
